...
UI_Form* Create_Form(char** Field_Descriptions, FIELDTYPE** Field_Types, char* Form_Title, size_t descriptions_length, size_t window_cols, size_t window_rows)
{
  ...
  for(index = 0; index <  out->descriptions_length; ++index)
  {
    ...
    if(out->Field_Types[index] == TYPE_ALPHA)
      set_field_type(out->Form_Fields[index], out->Field_Types[index], 0,0,300);
  }
  ...
  int label_margin = 2 + Get_Longest_String_Length(out->Field_Descriptions, out->descriptions_length);
  mvwprintw(out->Form_Window,1,2,out->Form_Title);
  ...
  for(index = 0; index <  out->descriptions_length; ++index)
    mvwprintw(out->Form_Window,index*2+4,2,out->Field_Descriptions[index]);
